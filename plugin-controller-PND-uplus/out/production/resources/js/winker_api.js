/**
    * wss 를 지정하지 않으면 기본 ws로 처리된다. 페이지가 기본 https를 사용할 때는 반드시 wss로 처리해야한다.
	...
	1. 통계 열기/닫기
	   ...
	   type은 숫자로 지정하며 ACDQ/VDN은 2, 상담원Person은 3, AgentGroup은 5 입니다.
	2. 조직 정보 요청
	  2.0 조직 정보 요청 유형
	  ...
	  2.5 ACDQ/VDN 처리를 위한 Switch 요청
		  ...
		  * 업무 센터 목록은 Switch와 별개로 상담원의 위치에 따라 구분될 수 있으며 Switch 갯수와는 다를 수 있습니다.
...
function winkWebSocket()
{
	...
	this.wLoginMode	= WLoginMode[WLoginModeType.LoginModeWink] ;//"LoginModeWink" ; //		// Winker 서버에 접속하는 client의 접속 모드이다. Wink/WinkAdmin 두개가 존재한다. 상담원는 Wink/관리자는 WinkAdmin이나 기본 Wink로 고정.
	this.wLoginCenter= null ;
	
	this.wSerial		= 10000 ;			// 시작은 10000 이고 99999를 넘으면 다시 10000으로 reset
	
	this.wCallbackEventFunction = null ;
	this.wCallbackLogFunction = null ;
	...
	this.wConnect = function(strHostP, intPortP, strHostB, intPortB, eventCallback, logCallback, consoleFlag, wssFlag)
	{
		...
		if ( this.wStatMap == null ) this.wStatMap = new WinkMap() ;
	...
	this.wConnectExt = function(connectFlag, wobject)
	{
		...
		// WebSocket.CLOSING		2
		// WebSocket.CLOSED		3		
		if ( this_.wWebSocket != null ) //&& this.wWebSocket.readyState == WebSocket.OPEN )
		{
		...
		try
		{
			var webSocket = new WebSocket(this_.wUriC);
			
			if(webSocket != null  && typeof webSocket != "undefined")
			{
				this_.wWebSocket = webSocket;
				...
				this_.wWebSocket.onmessage = function(msg)
				{
					...
					if ( decodedString.charAt(decodedString.length-1 ) == LST_DIM || decodedString.charAt(decodedString.length-1 ) == DATA_ETX ) // 끝에 도달한거다.
					{
						...
						if ( this_.wStoredBuffer != null && this_.wStoredBuffer.byteLength > 0)
						{
							...
							decodedString = this_.arr2Str(tempIntArr) ;
						} else
						{
				...
				this_.wWebSocket.onclose = function()
				{
					...
					if ( this_.wReconnectFlag == true )
					{
						...
						//this_.wTimerID = setTimeout(this_.wConnectExt(null,this_), this_.wReconnectInterval);
                 ...
                 * websocket error
                 */
				this_.wWebSocket.onerror = function() {
                    this_.wCallback(WMessageType.EventError, "EventID=<"+ WMessageType.EventError +"/>;EventName=<" + WMessage[WMessageType.EventError] + "/>") ;
				};
			}
		} catch (e)
		{
			wWinkerErr = e.message;
			console.log("웹소켓 접속요청 중 에러 발생 : " + wWinkerErr);
			throw new Error(wWinkerErr);
		}
	} ;
	...
	this.wDisconnect = function() {
		...
		if ( this.wWebSocket != null && this.wWebSocket.readyState != WebSocket.CLOSED ) {	// 2021-03-25 CLOSED 상태가 아니면 close한다. 그래야 connecting 상태도 close 된다.
	...
	{
		if ( strRequest.length > MAX_SIZE )
		{
			this.wLog("wSend","Data to send is to large....[" + strRequest.length + " > " + MAX_SIZE + "]");
	...
	this.wSendExt = function(strRequest)
	{
		...
		if ( strRequest.length > MAX_SIZE )
		{
			this.wLog("wSendExt","Data to send is to large....[" + strRequest.length + " > " + MAX_SIZE + "]");
	 ...
	 *
	 * @param strObjectID	{required} Winker 서버에 요청할 통계 항목에 대한 object key값. agent의 UserName. AgentGroup 이름. VDN/Queue 번호.
	 *								   dbid로 처리하는 경우에는 dbid^type 을 넣는다. type 값은 	ACDQ/VDN은 2, 상담원Person은 3, AgentGroup은 5이다.
	 *								   즉 상담원에 대한 통계를 요청할 때는 dbid1^2 형식으로 넣어주면 된다.
	 ...
	 *								   dbid로 처리하는 경우에는 dbid^type 을 "↑" 구분자로 목록을 만든다. type 값은 	ACDQ/VDN은 2, 상담원Person은 3, AgentGroup은 5이다.
	 *								   즉 상담원에 대한 통계를 요청할 때는 dbid1^2↑dbid2^2 형식으로 넣어주면 된다.
	 ...
	 * 2024-06-05 재오픈시 전송하는 byte수를 제한한다.
	 *
	 * @return		요청 접수 ReferenceID. -1 이면 요청 실패.
	 */
/*
	...
	this.wReOpenStat = function()
	{
		...
		for(var index in objectIDs)
		{
			...
			if ( textRequest.length > MAX_SIZE )
			{
				this.wLog("wReOpenStat","StatReOpenRequest Data....(" + reopenObjectCount + ")[" + textRequest +"]");
				...
				textRequest = WMessageType.RequestOpenStat + MSG_DIM + this.wGetRefID() ;
		...
		this.wLog("wReOpenStat","ReOpen ends....(" + reopenedObjectCount + ")");
	 ...
	 *								   dbid로 처리하는 경우에는 dbid^type 넣는다. type 값은 	ACDQ/VDN은 2, 상담원Person은 3, AgentGroup은 5이다.
	 ...
	 *								   dbid로 처리하는 경우에는 dbid^type 을 STA_DIM 구분자로 목록을 만든다. type 값은 	ACDQ/VDN은 2, 상담원Person은 3, AgentGroup은 5이다.
	...
	{
		//this.wLog("wClearStatMap","Clear starts....");
		if ( this.wStatMap == null || this.wStatMap.size() < 1 ) return ;
		...
		//this.wLog("wClearStatMap","Clear ends....");
	...
	/**
	 * 상담원 목록을 조회하기 위해 Level1 조직 정보를 요청한다. 최종 상담원 그룹 목록을 받기 위해서는 Center -> Level1 -> Level2 -> AgentGroup -> AgentList 순서로 조회한다.
	...
	/**
	 * 상담원 그룹에 대해서 상담원 목록을 요청한다.
	 * dbid:type:name 값을 "," 구분자로 이어서 온다. 즉  즉 "상담원그룹이름:dbid:5~dbid:type:FirstName^LastName^EmpID^UserNamme,238:3:90278^김민경M_Test_1^KG5061^KG5061" 와 같이 나중에 event로 온다.
	 *    
	...
	{
		this.wLog("wGetSwitchList","Requesting Switch List....");
	...
	/**
	 * 서버에 한명의 상담원 생성을 요청한다.
	 *
	 * @param intOwnerDBID		{required} 생성할 상담원가 위치할 Person 하위의 folder dbid. 0이면 Person 폴더 최상위에 생성.
	 * @param strEmpID			{required} 생성할 상담원의 Agent EmpID=UserName
	 * @param strFirstName		{required} 생성할 상담원의 Agent First Name
	 * @param strLastName		{required} 생성할 상담원의 Agent Last Name
	 * @param strAgentLoginID	{required} 상담원에게 추가하고자 하는 AgentLoginID@PBX1. 여러개 하고자 할 경우 ',' 로 구분한다. 이건 없으면 생성된다. 있으면 있는거 사용된다.
	 ...
	 * @return		요청 접수 ReferenceID. -1 이면 요청 실패.
	 */
	this.wCreateAgent = function(intOwnerDBID, strEmpID, strFirstName, strLastName, strAgentLoginID, strSkillDBID, strSkillLevel)
	{
		this.wLog("wCreateAgent","Create agent....[" + intOwnerDBID + "," + strEmpID + "," + strFirstName+ "," + strLastName+ "," + strAgentLoginID + "," + strSkillDBID + "," + strSkillLevel + "]");
		
		if ( this.isValidInt(intOwnerDBID) == false ) 	intOwnerDBID = 0 ;
		if ( this.isValidStr(strEmpID) == false ) 		return -1 ;
		if ( this.isValidStr(strFirstName) == false )		return -1 ;
		if ( this.isValidStr(strLastName) == false )		return -1 ;
	
		var refid = this.wGetRefID() ;
		
		// empid1234:정기:조:12345@SSPBX,12345@JAPBX:skill1,skill2:1,2
		// empid1234:정기:조:*@SSPBX,*@JAPBX:skill1,skill2:1,2
		var textRequest = WMessageType.RequestCreateAgent
						+ MSG_DIM + refid
						+ MSG_DIM + intOwnerDBID
								  + ":" + strEmpID
								  + ":" + strFirstName
								  + ":" + strLastName
								  + ":" + strAgentLoginID
		...
		this.wLog("wCreateAgent","Create agent request sent....");
	 ...
	 * @return		요청 접수 ReferenceID. -1 이면 요청 실패.
	 */
	this.wUpdateAgent = function(intDBID, strEmpID, strEmpIDNew, strFirstName, strLastName)
	{
		this.wLog("wUpdateAgent","Update agent....[" + intDBID + "," + strEmpID + "," + strEmpIDNew + "," + strFirstName+ "," + strLastName + "]");
		
		if ( this.isValidInt(intDBID) == false ) intDBID = 0 ;
		if ( intDBID <= 0 && this.isValidStr(strEmpID) == false ) return -1 ;
	
		var refid = this.wGetRefID() ;
		
		var textRequest = WMessageType.RequestUpdateAgent
						+ MSG_DIM + refid
						+ MSG_DIM + intDBID
								  + ":" + strEmpID
								  + ":" + strEmpIDNew
		...
		this.wLog("wUpdateAgent","Update agent request sent....");
	 ...
	 * @return		요청 접수 ReferenceID. -1 이면 요청 실패.
	 */
	this.wDeleteAgent = function(intDBID, strEmpID)
	{
		this.wLog("wDeleteAgent","Delete agent....[" + intDBID + "," + strEmpID + "]");
		...
		var textRequest = WMessageType.RequestDeleteAgent
						+ MSG_DIM + refid
						+ MSG_DIM + intDBID
								  + ":" + strEmpID
								  ;
		...
		this.wLog("wDeleteAgent","Delete agent request sent....");
	 ...
	 * @return		요청 접수 ReferenceID. -1 이면 요청 실패.
	 */
	this.wDeleteAgentOnly = function(intDBID, strEmpID)
	{
		this.wLog("wDeletePersonOnly","Delete agent only....[" + intDBID + "," + strEmpID + "]");
		...
		var textRequest = WMessageType.RequestDeleteAgentOnly
						+ MSG_DIM + refid
						+ MSG_DIM + intDBID
								  + ":" + strEmpID
								  ;
		...
		this.wLog("wDeletePerson","Delete agent request sent....");
	 ...
	 * @return		요청 접수 ReferenceID. -1 이면 요청 실패.
	 */
	this.wAddAgentIDToAgent = function(intDBID, strEmpID, strAddAgentLoginID)
	{
		this.wLog("wAddAgentIDToAgent","Add AgentID to agent....[" + intDBID + "," + strEmpID + "," + strAddAgentLoginID + "]");
		...
		var textRequest = WMessageType.RequestAddAgentIDToAgent
						+ MSG_DIM + refid
						+ MSG_DIM + intDBID
								  + ":" + strEmpID
								  + ":" + strAddAgentLoginID
		...
		this.wLog("wAddAgentIDToAgent","Add AgentID to agent request sent....");
	...
	/**
	 * 서버에 한명의 상담원에 AgentID를 삭제 요청한다.
	 *
	 * @param intDBID				{required} AgentID를 삭제할 상담원의 Person DBID. intDBID/strEmpID 둘중에 하나는 필수 값
	 * @param strEmpID				{required} AgentID를 삭제할 상담원의 Person EmpID. intDBID/strEmpID 둘중에 하나는 필수 값
	 * @param strDelAgentLoginID	{required} 상담원에게 삭제하고자 하는 AgentID DBID 또는 AgentLoginID@PBX1.
	 *
	 * @return		요청 접수 ReferenceID. -1 이면 요청 실패.
	 */
	this.wDeleteAgentIDFromAgent = function(intDBID, strEmpID, strDelAgentLoginID)
	{
		this.wLog("wDeleteAgentIDFromAgent","Detele AgentID from agent....[" + intDBID + "," + strEmpID + "," + strDelAgentLoginID + "]");
		...
		var textRequest = WMessageType.RequestDeleteAgentIDFromAgent
						+ MSG_DIM + refid
						+ MSG_DIM + intDBID
								  + ":" + strEmpID
								  + ":" + strDelAgentLoginID
		...
		this.wLog("wDeleteAgentIDFromAgent","Detele AgentID from agent request sent....");
	 ...
	 * @return		요청 접수 ReferenceID. -1 이면 요청 실패.
	 */
	this.wCreateAgentID = function(strCreateAgentLoginID)
	{
		this.wLog("wCreateAgentID","Create AgentID....[" + strCreateAgentLoginID + "]");
		
		if ( this.isValidStr(strCreateAgentLoginID) == false ) return -1 ;
	
		var refid = this.wGetRefID() ;

		var textRequest = WMessageType.RequestCreateAgentID
						+ MSG_DIM + refid
						+ MSG_DIM + strCreateAgentLoginID
								  ;
	
		if ( ! this.wSendExt(textRequest) ) return -1 ;
		
		this.wLog("wCreateAgentID","Create AgentID request sent....");
	 ...
	 * AgentID를 삭제 요청한다.
	 *
	 * @param intDBID				{optional} 삭제할 AgentID DBID.
	 * @param strDelAgentLoginID	{optional} 삭제하고자 하는 AgentID@PBX1.
	 *
	 * @return		요청 접수 ReferenceID. -1 이면 요청 실패.
	 */
	this.wDeleteAgentID = function(intDBID, strDelAgentLoginID)
	{
		this.wLog("wDeleteAgentID","Delete AgentID....[" + intDBID  + "," + strDelAgentLoginID + "]");
		
		if ( this.isValidInt(intDBID) == false ) intDBID = 0 ;
		if ( intDBID <= 0 && this.isValidStr(strDelAgentLoginID) == false ) return -1 ;
		
		var refid = this.wGetRefID() ;

		var textRequest = WMessageType.RequestDeleteAgentID
						+ MSG_DIM + refid
						+ MSG_DIM + (intDBID==0?strDelAgentLoginID:intDBID)
								  ;
	
		if ( ! this.wSendExt(textRequest) ) return -1 ;
		
		this.wLog("wDeleteAgentID","Delete AgentID request sent....");
	 ...
	 *
	 * @param strAgentDBID	{required} 스킬을 추가할 상담원의 Person DBID 
	 * @param strSkillDBID	{required} 상담원에게 추가하고자 하는 스킬의 DBID
	 * @param strSkillLevel	{required} 상담원에게 추가하고자 하는 스킬의 level
	 *
	 * @return		요청 접수 ReferenceID. -1 이면 요청 실패.
	 */
	this.wAddSkill = function(strAgentDBID, strSkillDBID, strSkillLevel)
	{
		this.wLog("wAddSkill","Add skill level....[" + strAgentDBID + "," + strSkillDBID + "," + strSkillLevel + "]");
		
		if ( this.isValidInt(strAgentDBID) == false ) return -1 ;
		if ( this.isValidInt(strSkillDBID) == false ) return -1 ;
		if ( this.isValidInt(strSkillLevel) == false ) return -1 ;
	
		var refid = this.wGetRefID() ;
		
		var textRequest = WMessageType.RequestAddSkill + MSG_DIM + refid + MSG_DIM + strAgentDBID + ":" + strSkillDBID +":" +  strSkillLevel;
	
		if ( ! this.wSendExt(textRequest) ) return -1 ;
		
		this.wLog("wAddSkill","Add skill level request sent....");
	 ...
	 * @return		요청 접수 ReferenceID. -1 이면 요청 실패.
	 */
	this.wAddSkillMulti = function(strAgentDBID, strSkillDBID, strSkillLevel)
	{
		this.wLog("wAddSkill","Add skill level multi....[" + strAgentDBID + "," + strSkillDBID + "," + strSkillLevel + "]");
		
		if ( this.isValidInt(strAgentDBID) == false ) return -1 ;
		if ( this.isValidStr(strSkillDBID) == false ) return -1 ;
		if ( this.isValidStr(strSkillLevel) == false ) return -1 ;
		...
		this.wLog("wAddSkill","Add skill level multi request sent....");
	 ...
	 * @param strSkillDBID	{required} 상담원에게 변경하고자 하는 스킬의 DBID
	 * @param strSkillLevel	{required} 상담원에게 변경하고자 하는 스킬의 level
	 *
	 * @return		요청 접수 ReferenceID. -1 이면 요청 실패.
	 */
	this.wUpdateSkill = function(strAgentDBID, strSkillDBID, strSkillLevel)
	{
		this.wLog("wUpdateSkill","Update skill level....[" + strAgentDBID + "," + strSkillDBID + "," + strSkillLevel + "]");
		...
		var textRequest = WMessageType.RequestUpdateSkill + MSG_DIM + refid + MSG_DIM + strAgentDBID + ":" + strSkillDBID +":" +  strSkillLevel;
	
		if ( ! this.wSendExt(textRequest) ) return -1 ;
		
		this.wLog("wUpdateSkill","Update skill level request sent....");
	 ...
	 * @return		요청 접수 ReferenceID. -1 이면 요청 실패.
	 */
	this.wUpdateSkillMulti = function(strAgentDBID, strSkillDBID, strSkillLevel)
	{
		this.wLog("wUpdateSkill","Update skill level multi....[" + strAgentDBID + "," + strSkillDBID + "," + strSkillLevel + "]");
		...
		this.wLog("wUpdateSkill","Update skill level multi request sent....");
	 ...
	 * @param strSkillDBID	{required} 상담원에게 삭제하고자 하는 스킬의 DBID
	 *
	 * @return		요청 접수 ReferenceID. -1 이면 요청 실패.
	 */
	this.wDeleteSkill = function(strAgentDBID, strSkillDBID)
	{
		this.wLog("wDeleteSkill","Delete skill level....[" + strAgentDBID + "," + strSkillDBID + "]");
		...
		var textRequest = WMessageType.RequestDeleteSkill + MSG_DIM + refid + MSG_DIM + strAgentDBID + ":" + strSkillDBID ;
	
		if ( ! this.wSendExt(textRequest) ) return -1 ;
		
		this.wLog("wDeleteSkill","Delete skill level request sent....");
	 ...
	 * @return		요청 접수 ReferenceID. -1 이면 요청 실패.
	 */
	this.wDeleteSkillMulti = function(strAgentDBID, strSkillDBID)
	{
		this.wLog("wDeleteSkill","Delete skill level multi....[" + strAgentDBID + "," + strSkillDBID + "]");
		...
		this.wLog("wDeleteSkill","Delete skill level multi request sent....");
	 ...
	 * @param strLogoutDN	{required} 로그아웃을 시킬 대상 dn. 같은 번호의 내선이 여러 교환기에 있을 경우에는 dn:switchdbid:switchname 형식으로 dbid 또는 name 을 반드시 지정해야 한다.
	 *
	 * @return		요청 접수 ReferenceID. -1 이면 요청 실패.
	 */
	 ...
	 * @param strRequestDN	{required} 로그아웃을 요청하는 사람 dn
	 * @param strLogoutDN	{required} 로그아웃을 시킬 대상 dn. 같은 번호의 내선이 여러 교환기에 있을 경우에는 dn:switchdbid:switchname 형식으로 dbid 또는 name 을 반드시 지정해야 한다.
	 * @param intSWDBID		{optional} 로그아웃을 시킬 대상 dn이 속한 switch dbid.
	 * @param strSWName		{optional} 로그아웃을 시킬 대상 dn이 속한 switch name.
	 *
	 * @return		요청 접수 ReferenceID. -1 이면 요청 실패.
	 */
	this.wLogoutDNBySwitch = function(strRequestDN, strLogoutDN, intSWDBID, strSWName)
	{
		this.wLog("wLogoutDNBySwitch","LogoutRequest....[" + strRequestDN + "," + strLogoutDN + "," + intSWDBID + "," + strSWName + "]");
	
		var refid = this.wLogoutDN(strRequestDN, strLogoutDN + ":" + intSWDBID + ":" + strSWName) ;
		
		this.wLog("wLogoutDNBySwitch","LogoutRequest sent....");
	 ...
	 * 특정 내선을 Listen한다.
	 *
	 * @param strRequestDN	{required} Listen을 요청하는 사람 dn. 감청 권한 있는 전화번호.
	 * @param strListenDN	{required} Listen을 할 대상 dn. 통화중인 전화번호.
	 *
	 * @return		요청 접수 ReferenceID. -1 이면 요청 실패.
	 */
	...
	/**
	 * 2020.06-19 추가. 특정 내선을 Listen을 중지 요청한다. 실제로는 감청으로 통화중인 호를 끊기 요청하는 거다.
	 *
	 ...
	 * @return		요청 접수 ReferenceID. -1 이면 요청 실패.
	 */
	this.wStopListenDN = function(strRequestDN)
	{
		this.wLog("wStopListenDN","StopListenRequest....[" + strRequestDN  + "]");
		...
		this.wLog("wStopListenDN","StopListenRequest sent....");
	 ...
	 *
	 * @param strAgentDBID		{required} 상담원 그룹에 추가할 상담원 DBID. 여러 상담원을 추가하고자 할 경우에는 LST_DIM 로 구분자로 입력하면 된다.ex)101;102;103
	 * @param intAgentGroupDBID	{required} 상담원을 추가할 상담원 그룹 DBID
	 *
	 * @return		요청 접수 ReferenceID. -1 이면 요청 실패.
	 */
	this.wRequestAddAgentToGroup = function(strAgentDBID, intAgentGroupDBID)
	{
		this.wLog("wRequestAddAgentToGroup","AddAgentToGroupRequest....[" + strAgentDBID + "," + intAgentGroupDBID + "]");
		
		if ( typeof strAgentDBID == "undefined" || strAgentDBID == null) return -1 ;
		if ( this.isValidInt(intAgentGroupDBID) == false ) return -1 ;
	
		var refid = this.wGetRefID() ;
		
		var textRequest = WMessageType.RequestAddAgentToGroup + MSG_DIM + refid + MSG_DIM + strAgentDBID + ":" + intAgentGroupDBID ;
	
		if ( ! this.wSendExt(textRequest) ) return -1 ;
		
		this.wLog("wRequestAddAgentToGroup","AddAgentToGroupRequest sent....");
	 ...
	 *
	 * @param strAgentDBID		{required} 상담원 그룹에서 삭제할 상담원 DBID. 여러 상담원을 삭제하고자 할 경우에는 LST_DIM 로 구분자로 입력하면 된다.ex)101;102;103
	 * @param intAgentGroupDBID	{required} 상담원을 삭제할 상담원 그룹 DBID
	 *
	 * @return		요청 접수 ReferenceID. -1 이면 요청 실패.
	 */
	this.wRequestDeleteAgentFromGroup = function(strAgentDBID, intAgentGroupDBID)
	{
		this.wLog("wRequestDeleteAgentFromGroup","DeleteAgentFromGroupRequest....[" + strAgentDBID + "," + intAgentGroupDBID + "]");
		...
		var textRequest = WMessageType.RequestDeleteAgentFromGroup + MSG_DIM + refid + MSG_DIM + strAgentDBID + ":" + intAgentGroupDBID ;
	
		if ( ! this.wSendExt(textRequest) ) return -1 ;
		
		this.wLog("wRequestDeleteAgentFromGroup","DeleteAgentFromGroupRequest sent....");
	...
	/**
	 * 서버에 새로운 AgentGroup 생성을 요청한다.
	 *
	 * @param strAgentGroupName	{required} 생성할 상담원 그룹 이름
	 * @param intOwnerDBID		{required} 생성할 상담원 그룹이 속할 folder dbid
	 *
	 * @return		요청 접수 ReferenceID. -1 이면 요청 실패.
	 */
	this.wCreateAgentGroup = function(strAgentGroupName, intOwnerDBID)
	{
		this.wLog("wCreateAgentGroup","Create agent group....[" + strAgentGroupName + "," + intOwnerDBID + "]");
		
		if ( typeof strAgentGroupName == "undefined" || strAgentGroupName == null) return -1 ;
		if ( this.isValidInt(intOwnerDBID) == false ) return -1 ;
	
		var refid = this.wGetRefID() ;
		
		var textRequest = WMessageType.RequestCreateAgentGroup + MSG_DIM + refid + MSG_DIM + strAgentGroupName + ":" + intOwnerDBID;
	
		if ( ! this.wSendExt(textRequest) ) return -1 ;
		
		this.wLog("wCreateAgentGroup","Create agent group request sent....");
	...
	/*
	 * 서버에 AgentGroup 삭제를 요청한다.
	 *
	 * @param intAgentGroupDBID	{required} 삭제할 상담원 그룹 dbid
	 *
	 * @return		요청 접수 ReferenceID. -1 이면 요청 실패.
	 */
	this.wDeleteAgentGroup = function(intAgentGroupDBID)
	{
		this.wLog("wDeleteAgentGroup","Delete agent group....[" + intAgentGroupDBID + "]");

		if ( this.isValidInt(intAgentGroupDBID) == false ) return -1 ;
	
		var refid = this.wGetRefID() ;
		
		var textRequest = WMessageType.RequestDeleteAgentGroup + MSG_DIM + refid + MSG_DIM + intAgentGroupDBID ;
	
		if ( ! this.wSendExt(textRequest) ) return -1 ;
		
		this.wLog("wDeleteAgentGroup","Delete agent group request sent....");
	...
	/*
	 * 서버에 AgentGroup 이름 변경을 요청한다.
	 *
	 * @param intAgentGroupDBID	{required} 이름을 변경할 상담원 그룹 dbid
	 * @param strAgentGroupName	{required} 상담원 그룹 새 이름
	 *
	 * @return		요청 접수 ReferenceID. -1 이면 요청 실패.
	 */
	this.wUpdateAgentGroup = function(intAgentGroupDBID, strAgentGroupName)
	{
		this.wLog("wUpdateAgentGroup","Update agent group....[" + intAgentGroupDBID + "," + strAgentGroupName + "]");
		...
		var textRequest = WMessageType.RequestUpdateAgentGroup + MSG_DIM + refid + MSG_DIM + intAgentGroupDBID + ":" + strAgentGroupName;
	
		if ( ! this.wSendExt(textRequest) ) return -1 ;
		
		this.wLog("wUpdateAgentGroup","Update agent group request sent....");
	 ...
	 * @return		요청 접수 ReferenceID. -1 이면 요청 실패.
	 */
	this.wUpdateAnnexOption = function(strDBID, strSection, strOption, strValue)
	{
		this.wLog("wUpdateAnnexOption","UpdateAnnexOption....[" + strDBID + "," + strSection + "," + strOption + "," + strValue + "]");
		
		if ( this.isValidInt(strDBID) == false ) return -1 ;
		if ( this.isValidStr(strSection) == false ) return -1 ;
		if ( this.isValidStr(strOption) == false ) return -1 ;
		if ( this.isValidStr(strValue) == false ) return -1 ;
		...
		this.wLog("wUpdateAnnexOption","UpdateAnnexOption sent....");
	 ...
	 * @return String
	 */
	this.wGetEventAttrValue = function(key, keyValList)
	{
		var index1 = -1 ;
		var index2 = -1 ;
		var retstr = null ;
	...
	/**
	 * 숫자인지 확인한다. undefined/null 일 경우는 false이다.
	 *
	...
	/**
	 * 숫자를 반환한다. undefined, null 일 경우는 -1을 반환한다. String일 경우에는 숫자로 변환해서 반환한다.
	 *
	 * @param intData		{required} 숫자
	 *
	 * @return		숫자일 경우 true. 아니면 false이다.
	 */
	this.checkValidInt = function(intData)
	{
		if ( typeof intData == "number" ) return intData ;
		if ( typeof intData != "undefined" && intData != null )
		{
			if ( typeof intData == "string" && ( intData = intData.trim()) != "" && isNaN(intData) == false ) return parseInt(intData) ;
		}
