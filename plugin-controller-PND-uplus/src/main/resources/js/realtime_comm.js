...
function depAndRoles() {
    ...
    try {
        if (page == 'AGENT_POS' && parent.Dec.userInfo.uplus.damdangs) {
            return;
        }
        if (page != 'AGENT_POS' && parent.Dec.userInfo.uplus.channels) {
            return;
        }
...
function getFilterData(options) {
    var page = options.page, type = options.type, data = options.data == undefined ? [] : (Array.isArray(options.data) ? options.data : new Array(options.data));
...
function getWProperties() {
    ...
    $.ajax({
        url         : fineServletURL + '/uplus/wprop',
        type        : 'GET',
        contentType : 'application/json',
        async       : false,
        success     : function(res) {
...
function wsError(isAlert) {
    ...
    if (isAlert) {
        ...
        BI.Msg.toast('통신 실패', {level: 'error'});
    }
}
