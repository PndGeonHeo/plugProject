var Dec = opener ? opener.parent.Dec : parent.Dec;
var BI = opener ? opener.parent.BI : parent.BI;

var userId = Dec ? Dec.personal.username : '';
var fineServletURL = Dec ? Dec.fineServletURL : '';


$(function() {
    depAndRoles();
});

/*
    부서/직무/역할 조회
*/
function depAndRoles() {
    $.ajax({
        url         : fineServletURL + '/uplus/depAndRoles',
        type        : 'GET',
        contentType : 'application/json',
        success     : function(res) {
            $.extend(Dec.userInfo, res);
        },
        error       : function(request,status,error) {
            BI.Msg.toast('권한 조회 실패');
        }
    });
}

/*
    array to tree (for treeselect)

    @param {Object} [options]
           {Array}  [options.data]   - 변환하고자 하는 배열
           {String} [options.value]  - value key
           {String} [options.name]   - name key
           {String} [options.parent] - parent key
*/
function arrayToTree(options) {
    var tree = [];
    var maps = {};

    $(options.data).each(function() {
        maps[this[options.value]] = {
            name: this[options.name],
            value: this[options.value],
            parent: this[options.parent],
            children: [],
            htmlAttr: {'data-value': this[options.value]}
        };
    });

    for (var id in maps) {
        if (maps[id]) {
            var elem = maps[id];
            if (elem['parent']) {
                maps[elem['parent']]['children'].push(elem);
            } else {
                tree.push(elem);
            }
        }
    }

    return tree;
}

/*
    org array to tree (for treeselect) -- 조직정보 tree 구조로 변환

    @param {Array} [data] - 조직정보
*/
function orgToTree(data) {
    var tree = [];
    var maps = {};

    $(data).each(function() {
        var name = this.org_name,
            value = this.depth == 1 ? this.org_name : this.org_cd,
            parent = this.depth == 2 ? this.channel_nm : this.hirk_org_cd,
            channel = this.channel_nm;

        maps[value] = {
            name: name,
            value: value,
            parent: parent,
            channel: channel,
            children: [],
            htmlAttr: {'data-value': value}
        };
    });

    for (var id in maps) {
        if (maps[id]) {
            var elem = maps[id];
            if (elem['parent']) {
                maps[elem['parent']]['children'].push(elem);
            } else {
                tree.push(elem);
            }
        }
    }

    return tree;
}

/*
    treeselect listSlotHtmlComponent - 전체선택 체크박스
*/
function getAllSelectSlot(var_ts) {
    var slot = document.createElement('a');
        slot.classList.add('treeselect-list__item');
        slot.classList.add('treeselect-allselect');
        slot.setAttribute('href', 'javascript:void(0);');
        slot.setAttribute('onclick', 'clickAllSelectSlot("' + var_ts + '");');

    var html =  '<div class="treeselect-list__item-checkbox-container ml-1">';
        html += '    <svg xmlns="http://www.w3.org/2000/svg" width="15" height="15" viewBox="0 0 25 25" fill="none" stroke="#000000" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polyline points="20 6 9 17 4 12"></polyline></svg>';
        html += '</div>';
        html += '<label class="treeselect-list__item-label">전체선택</label>';

    slot.innerHTML = html;

    return slot;
}

/*
    treeselect listSlotHtmlComponent - 전체선택 체크박스
    click 이벤트
*/
function clickAllSelectSlot(var_ts) {
    var ts = window[var_ts];
    if (!ts) return;

    var $slot = $(ts.listSlotHtmlComponent);
    $slot.toggleClass('treeselect-list__item--checked');

    var result = null;
    var isChecked = $slot.hasClass('treeselect-list__item--checked'); // 전체선택 체크여부

    if (ts.searchText) {
        // 검색 중인 경우 검색 결과에 대한 데이터만 처리
        var item = $(ts.srcElement).find('.treeselect-list__item:visible').not('.treeselect-allselect');
        var itemValue = Array.from(item.map((i, item) => String($(item).data('value'))));

        if (isChecked)
            result = [...new Set(ts.allValue.concat(itemValue))];
        else
            result = ts.allValue.filter(e => itemValue.indexOf(e) < 0);

    } else {
        if (isChecked) result = ts.options.map(e => e.value);
    }

    ts.updateValue(result);
}

/*
    treeselect listSlotHtmlComponent - 전체선택 체크박스
    선택값에 따른 전체선택 체크박스 제어
*/
function treeselectAll(ts) {
    if (!ts) return;

    var $item = $(ts.srcElement).find('.treeselect-list__item:visible').not('.treeselect-allselect');
    var $itemChecked = $(ts.srcElement).find('.treeselect-list__item--checked:visible').not('.treeselect-allselect');

    if ($item.length == $itemChecked.length)
        $(ts.listSlotHtmlComponent).addClass('treeselect-list__item--checked');
    else
        $(ts.listSlotHtmlComponent).removeClass('treeselect-list__item--checked');
}

/*
    muuri grid 각 box 영역 height 지정
    grid layout 라이브러리로 인해 빈 공간이 채워지는 것을 재조정하기 위함 (가시성)
*/
function setGridLayout(init) {
    if ($('.box-wrapper').length < 1) return;

    var top = $('.box-wrapper').eq(0).offset().top;

    // 화면에 보여지는 컬럼 수 체크
    var size = 0;
    $('.box-wrapper').each(function() {
        if (top == $(this).offset().top) {
            size++;
        } else {
            return false;
        }
    });

    // 해당하는 row에서 최대 높이를 가진 box 영역과 동일하게 변경
    if (col_size != size || init) {
        // row별로 진행
        for (var i=0; i<$('.box-wrapper').length; i+=size) {
            var subItems = $('.box-wrapper').slice(i, i+size);
            subItems.sort((a, b) => $(b).find('.box').height() - $(a).find('.box').height());

            var h = $(subItems[0]).find('.box').height();
            if (subItems.find('.item').length < 1) {
                h = 'auto';
            }

            $(subItems).each(function() {
                $(this).height(h);
            });
        }

        col_size = size;
        grid.refreshItems().layout();
    }
}

/*
    필터 저장
*/
function updateFilter(params) {
    var result = false;

    $.ajax({
        url         : fineServletURL + '/uplus/filter',
        type        : 'POST',
        async       : false,
        contentType : 'application/json',
        data        : JSON.stringify(params),
        success     : function(res) {
            if (res != 'success') {
                BI.Msg.toast('필터 저장 실패');
                result = false;
            }
            BI.Msg.toast('필터 저장 성공');
            result = true;
        },
        error       : function(request,status,error) {
            BI.Msg.toast('필터 저장 실패');
            result = false;
        }
    });

    return result;
}

/*
    필터 초기화
*/
function initFilter(o) {
    Dec.Msg.confirm({
        title: o.title ? o.title : document.title,
        message: o.message ? o.message : '해당 페이지의 필터가 전체 삭제됩니다.',
        callback: function(e) {
            if (e) deleteFilter(o.page);
        }
    });
}

/*
    필터 전체 삭제
*/
function deleteFilter(page) {
    var result = false;

    $.ajax({
        url         : fineServletURL + '/uplus/filter',
        type        : 'DELETE',
        async       : false,
        contentType : 'application/json',
        data        : page,
        success     : function(res) {
            if (res != 'success') {
                BI.Msg.toast('필터 초기화 실패');
                result = false;
            }
            BI.Msg.toast('필터 초기화 성공');
            result = true;
        },
        error       : function(request,status,error) {
            BI.Msg.toast('필터 초기화 실패');
            result = false;
        }
    });

    return result;
}

/*
    필터 데이터 가공

    @param {Object}        [options]
           {String|Number} [options.page]
           {String|Number} [options.type]
           {Array}         [options.data]
           {String|Number} [options.data.col]
           {String|Number} [options.data.value]
           {String|Number} [options.data.sort]
*/
function getFilterData(options) {
    var page = options.page, type = options.type, data = Array.isArray(options.data) ? options.data : new Array(options.data);

    var result = {
        userId: userId,
        page: page,
        type: type,
        dataList: []
    };

    data.forEach((e, i) => {
        if (!e.col) return;

        result.dataList.push({
            userId: userId,
            page: page,
            type: type,
            col: e.col,
            value: e.value,
            sort: e.sort ? e.sort : (i+1)
        });
    });

    return result;
}

/*
    초 -> 시:분:초
*/
function getHis(time) {
    var hour = Math.floor(time/3600);
    var min = Math.floor(time%3600/60);
    var sec = Math.floor(time%60);
    if(hour<10) hour = "0"+hour;
    if(min<10) min = "0"+min;
    if(sec<10) sec = "0"+sec;

    return hour+':'+min+':'+sec;
}

/*
    시:분:초 -> 초
*/
function getSS(time) {
    const arr = time.split(':');
    return (Number(arr[0]) * 3600) + (Number(arr[1]) * 60) + Number(arr[2]);
}

/*
    email 내 id 반환
*/
function getLoginId(str) {
    var str = $.trim(str);

    if (!str || str.indexOf('@') < 0)
        return str;

    return str.substring(0, str.indexOf('@'));
}

/*
    string to number
    NaN 또는 Infinity인 경우 0 반환
*/
function toNumber(value) {
    return isNaN(value) || !isFinite(value) ? 0 : Number(value);
}

/*
    winker websocket 연결 여부. OPEN(1) 상태인 경우에만 true
    > WebSocket.CONNECTING : 0
    > WebSocket.OPEN       : 1
    > WebSocket.CLOSING    : 2
    > WebSocket.CLOSED     : 3
*/
function isOpen(winker) {
    return winker && winker.wWebSocket && winker.wWebSocket.readyState == WebSocket.OPEN;
}

/*
    winker connection properties
*/
function getWProperties() {
    var type;

    $.ajax({
        url         : fineServletURL + '/uplus/env',
        type        : 'GET',
        contentType : 'application/json',
        async       : false,
        success     : function(res) {
            type = res;
        }
    });

    if (type == 'local')
        return WProperties_local;
    else
        return WProperties;
}

/*
    winker connecting event
    winker 연결 시도 중 중복 클릭 방지 및 로딩 표시
*/
function onLoading(o) {
    if (o) {
        $('.u-container').addClass('no-pointer-events');
        $('.u-container').find('.onloading').show();
        $('.u-container').find('.load-data').hide();

    } else {
        $('.u-container').removeClass('no-pointer-events');
        $('.u-container').find('.onloading').hide();
        $('.u-container').find('.load-data').show();
    }
}