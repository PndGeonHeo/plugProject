...
function init(resultData){
   // document.title = resultData.title.web;
    ...
    /*CommLogin.properties.splt.login.type = SSO */
    if (resultData?.splt?.login?.type === "sso") {
      addSSOButton();
    }

    // 로고 텍스트
    if(resultData?.title?.page){
        document.querySelector(".container .left .login-title").innerHTML = sanitizeTextWithLineBreaks(resultData.title.page);
    }
    // 로고 이미지 상단/텍스트 상단
    if(resultData?.splt?.login?.logo?.top === "Y"){
...
function addSSOButton(){
    ...
    ssoBtn.id = "ssoBtn";
    if (resultData?.splt?.login?.sso?.button?.content) {
      ssoBtn.innerText = resultData.splt.login.sso.button.content;
    }
    ssoBtn.classList.add("button");

    // 클릭 이벤트 (예: 다른 URL로 이동)
    ssoBtn.onclick = function () {
      window.location.href = resultData.splt.login.button.url;
    };
